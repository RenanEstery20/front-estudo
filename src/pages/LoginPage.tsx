...
export function LoginPage() {
  ...
  const helperText = useMemo(
    () =>
      isRegister
        ...
        : 'Use o seu email/senha cadastrados para acessar o sistema.',
    [isRegister],
  )
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (isRegister) {
        ...
        setMessage('Cadastro realizado. Agora faca login para acessar o sistema.')
        setMode('login')
        setPassword('')
        return
      }
  ...
  return (
    <main className="min-h-screen px-4 py-10">
      <div className="mx-auto grid min-h-[80vh] w-full max-w-5xl content-center items-start gap-8 lg:grid-cols-[1.1fr_0.9fr]">
        <section className="rounded-3xl border border-white/10 bg-white/5 p-8 backdrop-blur-md">
          ...
          <h1 className="text-3xl font-semibold tracking-tight text-white sm:text-4xl">
            Seja bem-vindo ao sistema de controle de caixa!
          </h1>
          <p className="mt-4 max-w-xl text-sm leading-6 text-slate-300">
            Caso já tenha acesso, faça login para visualizar suas movimentações financeiras. Se ainda não tem uma conta, cadastre-se para começar a usar o sistema.
          </p>
        </section>
